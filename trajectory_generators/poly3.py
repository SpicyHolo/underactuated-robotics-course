...
class Poly3(TrajectoryGenerator):
    def __init__(self, start_q, desired_q, T):
        ...
        """
        Please implement the formulas for a_0 till a_3 using self.q_0 and self.q_d
        ...
        self.a_0 = self.q_0
        self.a_1 = self.dq_0 + 3*self.q_0
        self.a_2 = 3*self.q_d - self.dq_d
        self.a_3 = self.q_d
    ...
    def generate(self, t):
        ...
        q_dot = (self.a_1 - 3*self.a_0) * (1 - t)**2 + 2*(self.a_2 - self.a_1) * t * (1 - t) + (3*self.a_3 - self.a_2) * t**2
        q_ddot = 2*(self.a_2 - 2*self.a_1 + 3*self.a_0) * (1 - t) + 2*(3*self.a_3 - 2*self.a_2 + self.a_1)*t
